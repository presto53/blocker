module Iptables_module
  ...
  def check_rule(source)
    run_cmd("#{@bin} -C #{@chain} -s #{source} -j DROP")
  end
  ...
  def add_chain
    ...
    if not POSIX::Spawn::Child.new("#{@bin} -C INPUT -p #{@protocol} -m multiport --dports #{@ports} -j #{@chain}").success?
  ...
  def remove_chain
    ...
    if POSIX::Spawn::Child.new("#{@bin} -L #{@chain}").success?
      ...
      if POSIX::Spawn::Child.new("#{@bin} -C INPUT -p #{@protocol} -m multiport --dports #{@ports} -j #{@chain}").success?
        raise "cannot delete INPUT rule for chain #{@chain}" if not POSIX::Spawn::Child.new("#{@bin} -D INPUT -p #{@protocol} -m multiport --dports #{@ports} -j #{@chain}").success?
      end
      # Delete chain
